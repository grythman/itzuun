...
export default function AdminUsersPage() {
  ...
  if (me.isLoading || admin.users.isLoading) return <LoadingState label="Loading users..." />;
  if (me.isError || !me.data) return <ErrorState label="Please sign in first." />;
  if (admin.users.isError) return <ErrorState label="Could not load users." />;
  ...
  return (
    <RoleGuard currentRole={me.data.role} requiredRole="admin" fallbackPath={withLocale("/auth")}>
      <section className="space-y-5">
        <h1 className="font-headline text-3xl font-extrabold text-surface-900">Admin Users</h1>
        <AppCard>
          {!users.length ? (
            ...
            <ul className="space-y-2">
              {users.slice(0, 20).map((item: any) => (
                <li key={item.id} className="rounded-xl border border-surface-200/60 p-3 text-[13px]">
                  ...
                  <p className="text-surface-500">Role: {item.role}</p>
                </li>
              ))}
            </ul>
          )}
