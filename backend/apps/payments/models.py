...
class Escrow(models.Model):
    ...
    project = models.OneToOneField(
        Project, on_delete=models.CASCADE, related_name="escrow"
    )
    ...
    class Meta:
        constraints = [
            models.CheckConstraint(
                check=Q(amount__gte=0), name="ck_escrow_amount_non_negative"
            ),
        ]
        indexes = [
            models.Index(
...
class LedgerEntry(models.Model):
    ...
    escrow = models.ForeignKey(
        Escrow, on_delete=models.CASCADE, related_name="ledger_entries"
    )
...
class Dispute(models.Model):
    project = models.ForeignKey(
        Project, on_delete=models.CASCADE, related_name="disputes"
    )
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(
                fields=["actor", "endpoint", "key"], name="uq_idempotency_actor_ep_key"
            ),
        ]
        indexes = [
            models.Index(
                fields=["endpoint", "created_at"], name="idx_idempo_endpoint_created"
            ),
        ]
    ...
    def save(self, *args, **kwargs):
        if self.pk is not None:
            raise ValidationError(
                "FinancialAuditLog is immutable and cannot be updated."
            )
